...
const maxInArray = (arr) => {
  // Solution code here...
  let maxVal = arr.reduce((acc, start) => {
...
const checkValues = (obj, value) => {
  ...
  return objArray.includes(value);
};
...
const hasChildrenValues = (arr, character) => {
  const localArrayVals = Object.values(arr);
  let hasChildren = null;
  let indexChar = arr.map(item => {
    return item.name.indexOf(character);
  });
  for (let i = 0; i < indexChar.length; i++) {
    if (indexChar[i] === 0) {
      hasChildren = localArrayVals[i].hasOwnProperty('children');
    }
  }
  return hasChildren;
};
