...
function transformToLis(obj) {
  ...
  for (let i = 0; i < objKeys.length; i++) {
    localArray.push(`<li>${objKeys[i]}: ${objVals[i]}</li>`);
  }
  return localArray;
}
...
const count = (target, input) => {
  // Solution code here...
  let temp = input.flat();
  let localNum = temp.reduce((acc, val) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    for (let j = 0; j < temp.length; j++) {
      if (typeof temp[j] === 'number' && temp[j] % 5 === 0) {
        tempArr.push(Math.pow(2, temp[j]));
      }
    }
    tempNumOnlyArr.push(tempArr);
  }
  return tempNumOnlyArr;
};
...
let findMaleAndFemale = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i].gender === 'male' || data[i].gender === 'female') {
      genderArr.push(data[i].name);
    }
  }
  let genderString = `${genderArr[0]}`;
  for (let i = 1; i < genderArr.length; i++) {
    genderString = (genderString + ` and ${genderArr[i]}`);
  };
  return genderString;
};
...
let findShortest = (data) => {
  // Solution code here...
  let lowestNameObj = data.reduce((acc, val) => {
    // console.log(acc, val);
    return (val.height > acc.height ? val : acc);
  }, data[0]);
  let lowestName = lowestNameObj.name;
  return lowestName;
};
