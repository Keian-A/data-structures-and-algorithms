...
const longestString = (arr) => {
  // Solution code here...
  let longestString = arr.reduce((next, string) => {
    let tempNextLength = next.split('').length;
    let tempStringLength = string.split('').length;
    let templongest = (tempStringLength > tempNextLength ? string : next);
    return templongest;
  }, '');
  let indexNum = -1;
  for (let i = 0; i < arr.length; i++) {
    if (longestString === arr[i]) {
      indexNum = i;
    }
  }
  return indexNum;
